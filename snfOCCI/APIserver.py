        ...
        if not req.environ.has_key('HTTP_X_AUTH_TOKEN'):
            ...
            if ASTAVOMS_IS_KEYSTONE:
                headers = [
                    ...
                    ('Www-Authenticate','Keystone uri=\'{astavoms_url}\''.format(astavoms_url=ASTAVOMS_URL))
                ]
            else:
                headers = [('Content-Type', 'text/html'),('Www-Authenticate','Keystone uri=\'https://okeanos-occi2.hellasgrid.gr:5000/main\'')]
            response(status,headers)
            print 'Ask for redirect to astavoms URL {astavoms_url}'.format(astavoms_url=ASTAVOMS_URL)
            return [str(response)]

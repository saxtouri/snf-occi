...
cat > $TEMP_DIR/json << EOF
{
	...
	"attributes": {
		"occi.core.title": "My Test Network",
...
cat > $TEMP_DIR/json << EOF
{
	...
	"attributes": {
		"occi.core.target": "${NETWORK_URL}",
		"occi.core.source": "${VM_URL}"
	}
}
EOF

CMD="${BASE_CMD} -X'POST' $OCCI_ENDPOINT/networklink/ -H'Content-Type: application/occi+json' -d @$TEMP_DIR/json"
echo $CMD
LINK_URL=$(eval $CMD)
